...
void main() {
  ...
  test('Successful predictLongRunning request and operation polling', () async {
    ...
    final mockClient = MockClient((request) async {
      print("MOCK CLIENT: ${request.method} ${request.url}");
      if (request.method == 'POST') {
        correctExecutionHeaders = request.headers['x-goog-api-key'] == 'test-key';
        final body = jsonDecode(request.body);
        
        correctExecutionPayload = body['instances'] != null &&
            body['instances'][0]['image'] != null &&
            body['instances'][0]['image']['bytesBase64Encoded'] != null &&
            body['instances'][0]['image']['mimeType'] != null;
            
        if (!correctExecutionPayload) {
          print("Payload incorrect: $body");
        }

        return http.Response(jsonEncode({"name": operationName}), 200);
      } else if (request.method == 'GET' && request.url.toString().contains(operationName)) {
      ...
      } else if (request.method == 'GET' && request.url.toString() == videoUri) {
        correctDownloadHeaders = request.headers['x-goog-api-key'] == 'test-key';
        downloadAttempted = true;
        return http.Response.bytes(videoBytes, 200);
      }

      print("Not found: ${request.method} ${request.url}");
    ...
    // Simulate background process executing and setting operation_name
    await Future.delayed(const Duration(milliseconds: 200));
    
    expect(correctExecutionHeaders, true);
    expect(correctExecutionPayload, true);
    
    // Simulate background process setting operation_name
