...
class ConcurrencyTestAgent extends Agent {
  ...
  ConcurrencyTestAgent({required super.toolbox, required super.permissionPolicy});
  ...
  Future<AgentPlan> formulatePlan(AgentSession session) async {
    ...
    if (!isWaitingForJob) {
      ...
      return AgentPlan(goal: 'test', steps: [
        AgentStep(id: 'step1', description: 'Test', action: AgentAction(toolId: 'test_job', riskLevel: ToolRiskLevel.safe, arguments: {}))
      ]);
    }
    return AgentPlan(goal: 'test', steps: []);
  }
...
void main() {
  group('Agent Concurrency and Jobs', () {
    test('Job without artifact completes and loop resumes', () async {
      final agent = ConcurrencyTestAgent(toolbox: DummyToolbox(), permissionPolicy: PermissionPolicy());
      final session = AgentSession(
        currentProject: NoemaProject(id: '1', idea: '', story: Story(title: '', scenes: [])),
        currentGoal: 'test',
      );
      ...
      expect(session.state, AgentSessionState.failed); // since formulatePlan returns 0 steps and no pending jobs
