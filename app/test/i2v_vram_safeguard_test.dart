...
void main() {
  test('I2V Workflow correctly applies VRAM safeguards (14 frames)', () async {
    ...
    expect(
      ...
      14,
      reason:
          "SVD video_frames should be reduced to 14 to save VRAM on balanced profile.",
    );

    // Test custom options
    adapter.applyOptions({'motion': 200, 'augmentation': 0.1});
    final motion = getInputByClass(
      workflow,
      'SVD_img2vid_Conditioning',
      'motion_bucket_id',
    );
    final aug = getInputByClass(
      workflow,
      'SVD_img2vid_Conditioning',
      'augmentation_level',
    );
