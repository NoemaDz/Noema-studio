...
class TimeoutMockProvider extends AsyncProvider {
  ...
  @override
  HardwareRequirements get hardwareRequirements => const HardwareRequirements(requiresGPU: false, minimumVRAMGB: 0);
...
void main() {
  group('JobMonitor Timeout Semantics Tests', () {
    ...
    test('timeout triggers provider cancellation and reaches failed state', () async {
      final monitor = JobMonitor(runner, manager, events, timeoutDuration: const Duration(seconds: 1));
      
      final job = Job(id: '1', providerId: 'timeout_mock', type: 'image', status: JobStatus.running);
      manager.add(job);
      ...
      if (duration > monitor.timeoutDuration) {
        await manager.cancelJob(
          ...
          error: JobError(
            code: 'TIMEOUT',
            message: 'Job exceeded maximum execution time of ${monitor.timeoutDuration.inMinutes} minutes.',
          ),
        );
      }
    ...
    test('queued timeout triggers provider cancellation and reaches failed state', () async {
      ...
      expect(provider.wasCancelled, isTrue); // Queued jobs might not have been sent to provider, but we attempt cancel anyway
      expect(job.status, JobStatus.failed);
      expect(job.error?.code, 'QUEUED_TIMEOUT');
    });

    test('timeout + completion race (completed status arrives right before timeout applies)', () async {
      JobMonitor(runner, manager, events, timeoutDuration: const Duration(seconds: 1));
      
      final job = Job(id: '1', providerId: 'timeout_mock', type: 'image', status: JobStatus.running);
      manager.add(job);
      
      job.startedAt = DateTime.now().subtract(const Duration(minutes: 10)); // Timeout condition met
    ...
    test('timeout cannot result in a later completed Job (terminal state enforces immutability)', () async {
      final job = Job(id: '1', providerId: 'timeout_mock', type: 'image', status: JobStatus.running);
      manager.add(job);
