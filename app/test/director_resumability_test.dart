...
void main() {
  group('Resumability & Lifecycle Safety Tests', () {
    test('Backward compatibility - loads correctly without stopped state (Test 5)', () {
      final json = {
        'id': 'test-123',
        'idea': 'Test idea',
      ...
      expect(project.projectState, GenerationState.draft); // Defaults correctly
    });
    ...
    test('Resume logic preserves UUID and state (Test 3)', () {
      final project = NoemaProject(
        id: 'stable-uuid',
        idea: 'Resume idea',
        story: Story(title: 'Saved', scenes: [
          Scene(id: 1, description: 'S1', imageState: GenerationState.completed)
        ]),
      );
    ...
    test('Complete Director AI resume flow (production resumes incomplete work)', () {
      final project = NoemaProject(
        id: 'stable-uuid',
        idea: 'Resume idea',
        story: Story(title: 'Saved', scenes: [
      ...
      project.images.add(GeneratedImage(jobId: 'job1', sceneId: 1, prompt: 'p1'));
      // Scene 2 had an incomplete image
      project.images.add(GeneratedImage(jobId: 'job2', sceneId: 2, prompt: 'p2'));
      ...
      expect(project.images.length, 1, reason: "Incomplete image artifact should be purged for regeneration");
      expect(project.images.first.sceneId, 1, reason: "Completed scene artifact MUST be preserved");
      expect(project.images.first.jobId, 'job1');
      expect(project.id, 'stable-uuid', reason: "Project UUID must remain stable");
    });
  });
}
