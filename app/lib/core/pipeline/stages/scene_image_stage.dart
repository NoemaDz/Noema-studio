...
class SceneImageStage implements PipelineStage {
  ...
  Future<void> run(NoemaProject project) async {
    project.images.clear();
    debugPrint(
      "SceneImageStage: Starting. Cleared project.images. Story has ${project.story.scenes.length} scenes.",
    );
    ...
    for (final scene in project.story.scenes) {
      ...
      try {
        debugPrint(
          "SceneImageStage: Running ImageWorkflow for scene ${scene.id}...",
        );
        final result = await engine.runWithContext(workflow, context);
        ...
        if (job != null) {
          job.metadata["title"] = "Generating Scene ${scene.id} Image";
          debugPrint(
            "SceneImageStage: Workflow returned job ${job.id} for scene ${scene.id}. Adding to project.",
          );
          project.jobs.add(job);
          project.images.add(
            GeneratedImage(
          ...
          debugPrint(
            "SceneImageStage: ERROR - Workflow returned null job for scene ${scene.id}!",
          );
        }
      } catch (e) {
        debugPrint(
          "SceneImageStage: ERROR running workflow for scene ${scene.id}: $e",
        );
        rethrow;
      }
    }
    debugPrint(
      "SceneImageStage: Finished. project.images contains ${project.images.length} images.",
    );
  }
}
