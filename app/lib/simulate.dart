...
import 'package:app/infrastructure/comfyui/comfyui_plugin.dart';
import 'package:app/infrastructure/ollama/ollama_plugin.dart';
import 'package:app/infrastructure/openai/openai_plugin.dart';
import 'package:app/infrastructure/tts/flutter_tts_plugin.dart';
import 'package:app/infrastructure/ffmpeg/ffmpeg_plugin.dart';
import 'package:app/core/plugins/core_pipeline_plugin.dart';
import 'package:app/core/plugins/ingestion_plugin.dart';

import 'package:app/models/job.dart';
import 'package:app/application/project_synchronizer.dart';

import 'package:app/main.dart' show noema;
import 'package:app/core/noema_project.dart';
...
void main() async {
  ...
  try {
    ...
    print("Project generated successfully! Waiting for background jobs (Images, Audio, Video)...");
    ...
    while (project.finalVideoPath == null) {
      if (project.jobs.isEmpty) {
         ...
         if (emptyJobsCounter > 5) {
            print("Error: No jobs were generated. Check the LLM output or JSON parsing.");
            break;
         }
      } else {
         emptyJobsCounter = 0;
      ...
      bool allJobsDone = project.jobs.isNotEmpty && project.jobs.every((j) => j.status == JobStatus.completed);
      if (allJobsDone && project.finalVideoPath == null && project.jobs.any((j) => j.type == "video_compile")) {
      ...
      print("Waiting... Active jobs: ${project.jobs.where((j) => j.status == JobStatus.running || j.status == JobStatus.queued).length} / ${project.jobs.length}");
    }
