...
class ComfyUIDriver {
  String get baseUrl => noema.bootstrap.appSettings.comfyUIUrl;
  ...
  try {
    ...
    debugPrint("ComfyUIDriver: Failed to load $workflowPath ($e), falling back to text_to_image_api.json");
  ...
  if (useIpAdapter) {
    for (int i = 0; i < characters.length; i++) {
      ...
      if (json.containsKey(imageNodeId) && json[imageNodeId]["class_type"] == "LoadImage") {
        ...
        final String filename = uri.queryParameters["filename"] ?? "image.png";
  ...
  final response = await http.get(
    Uri.parse("$baseUrl/history/$jobId"),
  );
  ...
  try {
    ...
    final outputs =
        data[jobId]["outputs"] as Map<String, dynamic>;
    ...
    for (final node in outputs.values) {
      ...
      if (node["images"] != null || node["gifs"] != null) {
        ...
        final filename = isVideo ? node["gifs"][0]["filename"] : node["images"][0]["filename"];
        ...
        debugPrint("ComfyUIDriver: Download status ${assetResponse.statusCode}");
