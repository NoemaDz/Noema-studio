  ...
  static String buildFilter(
    ...
    switch (effectName.toLowerCase()) {
      case 'zoom_in':
        ...
        return "[0:v]$cropFilter,zoompan=z='min(zoom+0.0015,1.5)':d=1000:x='iw/2-(iw/zoom)/2':y='ih/2-(ih/zoom)/2':s=${width}x$height:fps=25";
      case 'zoom_out':
        // Zoom out slowly from center. (zoompan requires z >= 1, so we start at 1.5 and zoom out to 1)
        return "[0:v]$cropFilter,zoompan=z='if(eq(on,1),1.5,max(1.0,zoom-0.0015))':d=1000:x='iw/2-(iw/zoom)/2':y='ih/2-(ih/zoom)/2':s=${width}x$height:fps=25";
      case 'pan_right':
        // Pan from left to right. To pan, we must be zoomed in a bit first.
        return "[0:v]$cropFilter,zoompan=z='1.2':d=1000:x='x+1':y='ih/2-(ih/zoom)/2':s=${width}x$height:fps=25";
      case 'pan_left':
      ...
      default:
        ...
        return "[0:v]$cropFilter,zoompan=z='1':d=1000:x='iw/2-(iw/zoom)/2':y='ih/2-(ih/zoom)/2':s=${width}x$height:fps=25";
    }
  }
