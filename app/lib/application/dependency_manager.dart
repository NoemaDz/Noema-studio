...
class DependencyManager {
  ...
  Future<void> ensureDependencies(Function(String) onProgress) async {
    ...
    final binPath = PlatformPaths.instance.binariesDirectory;
    ...
    if (hasGit) {
      ...
      final localGitPath = _getPortableGitPath(binPath);
      if (await File(localGitPath).exists()) {
        _gitPath = localGitPath;
        onProgress("Portable Git found locally.");
      } else {
        await _downloadPortableGit(binPath, onProgress);
        _gitPath = localGitPath;
      }
    }
  ...
  Future<void> _downloadPortableGit(
    String binPath,
    ...
    if (Platform.isWindows) {
      ...
      const url =
          "https://github.com/git-for-windows/git/releases/download/v2.41.0.windows.1/MinGit-2.41.0-64-bit.zip";
      ...
      throw Exception(
        "Git is not installed! Please install Git on your system first (e.g., sudo apt install git).",
      );
    }
  }

  Future<void> _downloadPortableFfmpeg(
    String binPath,
    Function(String) onProgress,
  ) async {
    if (Platform.isWindows) {
      ...
      final innerFolder =
          Directory(extractPath).listSync().firstWhere((e) => e is Directory)
              as Directory;
      ...
      try {
        ...
        final extractedDirs = Directory(binPath)
            .listSync()
            .whereType<Directory>()
            .where((d) => p.basename(d.path).startsWith('ffmpeg'));
      ...
      throw Exception(
        "Please install FFmpeg manually via Homebrew: brew install ffmpeg",
      );
    }
  }
